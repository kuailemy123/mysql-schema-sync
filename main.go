...
import (
	"flag"
	"fmt"
	"github.com/kuailemy123/mysql-schema-sync/internal"
	"log"
	"os"
	"strings"
)
...
var tables_ignore = flag.String("tables_ignore", "", "Ignore the checked table name\n\teg : product_base,order_*")
...
func main() {
    ...
    if *tables_ignore != "" {
		_ts := strings.Split(*tables_ignore, ",")
		for _, _name := range _ts {
			_name = strings.TrimSpace(_name)
			if _name != "" {
